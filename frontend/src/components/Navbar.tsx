    ...
    <header className="sticky top-0 z-20 border-b border-gray-200 bg-white/90 backdrop-blur">
      <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1">
          <NavLink to="/" className={({ isActive }: { isActive: boolean }) => `${linkBase} ${isActive ? linkActive : linkInactive}`}>Home</NavLink>
          <NavLink to="/health" className={({ isActive }: { isActive: boolean }) => `${linkBase} ${isActive ? linkActive : linkInactive}`}>Health</NavLink>
          <NavLink to="/about" className={({ isActive }: { isActive: boolean }) => `${linkBase} ${isActive ? linkActive : linkInactive}`}>About</NavLink>
        </nav>
      </div>
    </header>
  );
